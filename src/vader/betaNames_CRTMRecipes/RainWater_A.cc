...
#include "vader/betaNames_CRTMRecipes/RainWater.h"

namespace vader {

// -------------------------------------------------------------------------------------------------
// Static attribute initialization
const char RainWater_A::Name[] =
    "RainWater_A";
const oops::Variables RainWater_A::Ingredients{
    std::vector<std::string>{
      "air_temperature", "air_pressure", "relative_humidity", "air_pressure_at_surface"}};

// Register the maker
static RecipeMaker<RainWater_A>
  makerRainWater_A_(
      RainWater_A::Name);

// -------------------------------------------------------------------------------------------------

RainWater_A::RainWater_A(
    const Parameters_ & params, const VaderConfigVars & configVariables) {
  oops::Log::trace() << "RainWater_A::"
                     << "RainWater_A" << std::endl;
}

std::string RainWater_A::name() const {
  return RainWater_A::Name;
}

oops::Variable RainWater_A::product() const {
  return oops::Variable{"rain_water"};
}

oops::Variables RainWater_A::ingredients() const {
  return RainWater_A::Ingredients;
}

oops::Variables RainWater_A::trajectoryVars() const {
  return oops::Variables{std::vector<std::string>{
    "air_temperature", "air_pressure", "relative_humidity", "air_pressure_at_surface"}};
}

size_t RainWater_A::productLevels(
    const atlas::FieldSet & afieldset) const {
  return afieldset.field("air_temperature").shape(1);
}

atlas::FunctionSpace RainWater_A::productFunctionSpace(
    const atlas::FieldSet & afieldset) const {
  return afieldset.field("air_temperature").functionspace();
}
// -------------------------------------------------------------------------------------------------

void RainWater_A::executeNL(atlas::FieldSet & afieldset) {
  oops::Log::trace() << "RainWater_A::executeNL starting"
                     << std::endl;

  const int nlevels = afieldset.field("air_temperature").shape(1);
  const double RH_crit_base   = 0.85;
  const double RH_crit_slope  = 0.10;  // slope of RH threshold vs P/Ps; empirical
  const double rain_water_max = 5e-5;  // peak rain water mixing ratio [kg kg-1]; empirical
  const double T_freeze       = 273.15;
  const double T_warm         = 283.15;
      ...
      [&](const auto temp_col,
          ...
          for (int jl = 0; jl < nlevels; ++jl) {
            ...
            if (T > T_freeze && RH > RH_crit) {
              ...
              const double temp_factor = std::min(1.0, (T - T_freeze) / (T_warm - T_freeze));
              rain_col(jl) = rain_water_max * RH_excess / (1.0 - RH_crit) * temp_factor;
            } else {
              rain_col(jl) = 0.0;
            }
          }
  ...
  oops::Log::trace() << "RainWater_A::executeNL done"
                     << std::endl;
}

// -------------------------------------------------------------------------------------------------

void RainWater_A::executeTL(atlas::FieldSet & afieldsetTL,
                             const atlas::FieldSet & afieldsetTraj) {
  oops::Log::trace() << "RainWater_A::executeTL starting" << std::endl;
            ...
            if (T > T_freeze && RH > RH_crit && RH_excess > 0.0) {
              ...
              if (temp_factor > 0.0) {
                ...
                const double F_tl        = (RH_excess_tl * denom + RH_excess * RH_crit_tl)
                                           / (denom * denom);
                const double tf_tl = ((T - T_freeze) < (T_warm - T_freeze) ? 1.0 : 0.0)
                                     / (T_warm - T_freeze) * temp_tl_col(jl);
                rain_tl_col(jl) = rain_water_max * (F_tl * temp_factor + F * tf_tl);
              } else {
                rain_tl_col(jl) = 0.0;
              }
            } else {
  ...
  oops::Log::trace() << "RainWater_A::executeTL done" << std::endl;
}

// -------------------------------------------------------------------------------------------------

void RainWater_A::executeAD(atlas::FieldSet & afieldsetAD,
                             const atlas::FieldSet & afieldsetTraj) {
  oops::Log::trace() << "RainWater_A::executeAD starting" << std::endl;
      ...
      [&](const auto temp_col,
          ...
          for (int jl = 0; jl < nlevels; ++jl) {
            ...
            if (T > T_freeze && RH > RH_crit && RH_excess > 0.0) {
              ...
              if (temp_factor > 0.0) {
                const double F    = RH_excess / denom;
                const double r_ad = rain_ad_col(jl);

                const double F_ad  = rain_water_max * temp_factor * r_ad;
                const double tf_ad = rain_water_max * F * r_ad;
                ...
                if ((T - T_freeze) < (T_warm - T_freeze))
                  temp_ad_col(jl) += 1.0 / (T_warm - T_freeze) * tf_ad;
  ...
  oops::Log::trace() << "RainWater_A::executeAD done" << std::endl;
}
