...
void GeopotentialAtInterface_A::executeNL(atlas::FieldSet & afieldset)
{
    ...
    [&](auto phi_col, auto tv_col, auto ln_p_col, auto ln_p_int_col, auto phi_int_col) {
...
void GeopotentialAtInterface_A::executeTL(atlas::FieldSet & afieldsetTL,
    ...
    util::for_each_column(
    [&](auto phi_tl_col, auto tv_tl_col, auto ln_p_col, auto ln_p_int_col, auto phi_int_tl_col) {
        double dln_top = ln_p_col(0) - ln_p_int_col(0);
    ...
    phi_tl, tv_tl, ln_p, ln_p_int, phi_int_tl);
...
void GeopotentialAtInterface_A::executeAD(atlas::FieldSet & afieldsetAD,
    ...
    util::for_each_column(
    [&](auto phi_ad_col, auto tv_ad_col, auto ln_p_col, auto ln_p_int_col, auto phi_int_ad_col) {
        for (int k = nint - 2; k >= 0; --k) {
            double dln = ln_p_int_col(k + 1) - ln_p_int_col(k);
            double phi_int_ad_kp1 = phi_int_ad_col(k + 1);
            double tv_layer_ad = -rdry * dln * phi_int_ad_kp1;

            phi_int_ad_col(k) += phi_int_ad_kp1;
            tv_ad_col(k)     += 0.5 * tv_layer_ad;
            tv_ad_col(k + 1) += 0.5 * tv_layer_ad;

            phi_int_ad_col(k + 1) = 0.0;
        }

        double dln_top = ln_p_col(0) - ln_p_int_col(0);
        double phi_int_ad_0 = phi_int_ad_col(0);
        double tv_layer_ad_0 = rdry * dln_top * phi_int_ad_0;

        phi_ad_col(0) += phi_int_ad_0;
        tv_ad_col(0) += 0.5 * tv_layer_ad_0;
        tv_ad_col(1) += 0.5 * tv_layer_ad_0;

        phi_int_ad_col(0) = 0.0;
    },
    phi_ad, tv_ad, ln_p, ln_p_int, phi_int_ad);
